...
def build_reportes_context(request):
    ...
    for i in range(5, -1, -1):
        ...
        meses.append({"key": f"{y:04d}-{m:02d}", "label": inicio.strftime("%b %Y"), "start": inicio, "end": fin})
    ...
    ventas_qs = (
        Cotizacion.objects.filter(fecha__gte=meses[0]["start"], fecha__lt=meses[-1]["end"], completada=True)
    ...
    for m in meses:
        ...
        ventas_mes.append({"mes": m["label"], "total": float(row["total"]) if row else 0, "cantidad": row["cantidad"] if row else 0})

    facturado_mes = (
        Cotizacion.objects.filter(fecha__gte=rango_inicio, fecha__lt=rango_fin, completada=True).aggregate(t=Sum("total"))["t"] or 0
    )
    monto_cotizado_mes = (
        Cotizacion.objects.filter(fecha__gte=rango_inicio, fecha__lt=rango_fin).aggregate(t=Sum("total"))["t"] or 0
    )
    ...
    top_productos = (
        CotizacionItem.objects.filter(
            cotizacion__completada=True, cotizacion__fecha__gte=rango_inicio, cotizacion__fecha__lt=rango_fin
        )
    ...
    tipo_data = {
        "presupuestos": Cotizacion.objects.filter(
            tipo_documento="presupuesto", fecha__gte=rango_inicio, fecha__lt=rango_fin
        ).count(),
        "recibos": Cotizacion.objects.filter(tipo_documento="recibo", fecha__gte=rango_inicio, fecha__lt=rango_fin).count(),
    }
    ...
    return {
        ...
        "top_clientes_json": json.dumps(
            [{"nombre": c["cliente__nombre"], "total": float(c["total"]), "cantidad": c["cantidad"]} for c in top_clientes]
        ),
        "top_productos_json": json.dumps(
            [{"nombre": p["producto__nombre"], "veces": p["veces"], "total": float(p["total_vendido"])} for p in top_productos]
        ),
